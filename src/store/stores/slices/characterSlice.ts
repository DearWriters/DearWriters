...
export const createCharacterSlice: StateCreator<StoreState, [], [], CharacterSlice> = (set) => ({
  ...
  updateCharacterRelationship: (charAId, charBId, description) => set((state) => {
    return {
      characters: state.characters.map(c => {
        if (c.id === charAId) {
          return { ...c, relationships: { ...(c.relationships || {}), [charBId]: description } };
        }
        if (c.id === charBId) {
          return { ...c, relationships: { ...(c.relationships || {}), [charAId]: description } };
        }
        return c;
      }),
      lastModified: Date.now()
  ...
  removeCharacterRelationship: (charAId, charBId) => set((state) => {
    return {
      characters: state.characters.map(c => {
        if (c.id === charAId) {
          const newRels = { ...c.relationships };
          delete newRels[charBId];
          return { ...c, relationships: newRels };
        }
        if (c.id === charBId) {
          const newRels = { ...c.relationships };
          delete newRels[charAId];
          return { ...c, relationships: newRels };
        }
        return c;
      }),

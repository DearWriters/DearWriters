...
export const createWorkSlice: StateCreator<StoreState, [], [], WorkSlice> = (set, get) => ({
  addWork: (title) => set((state) => {
    const newWork: Work = { id: uuidv4(), title, createdAt: Date.now(), order: state.works.length, coverImage: '' };
  ...
  setActiveWork: (workId) => set((state) => {
    if (workId === null) return { activeWorkId: null, activeDocumentId: null };

...
export const SlashCommandMenu: React.FC<SlashCommandMenuProps> = ({ onClose, onSelect, position }) => {
  ...
  const actions = [
    { name: '转换块', icon: Highlighter, action: 'convert' },
    { name: '向上合并', icon: ArrowUpToLine, action: 'merge' },
    { name: '拆分场景', icon: Scissors, action: 'split' },
    { name: '比较', icon: GitCompare, action: 'compare' },
    { name: '删除', icon: Trash2, action: 'delete' },
  ];

...
const TAG_COLORS = [
  ...
  'bg-wood-100 text-wood-800 border-wood-200',
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 overflow-y-auto flex-1 max-w-3xl mx-auto w-full custom-scrollbar">
      <div className="mb-6 sm:mb-8">
        <h3 className="text-xl sm:text-2xl font-bold text-stone-800 tracking-tight">管理标签</h3>
        <p className="text-xs sm:text-sm text-stone-500 mt-1">创建和组织标签以对时间线事件进行分类。</p>
      </div>
      ...
      <form onSubmit={handleAddTag} className="mb-8 sm:mb-10 space-y-4 sm:space-y-5 bg-white p-4 sm:p-6 rounded-2xl border border-stone-200/80 shadow-sm">
        <div>
          <label className="text-[10px] sm:text-xs font-bold text-stone-500 uppercase tracking-widest block mb-2">新标签名称</label>
          <input
            type="text"
            value={newTagName}
            onChange={(e) => setNewTagName(e.target.value)}
            placeholder="例如：重要，支线A"
            className="w-full text-xs sm:text-sm bg-stone-50/50 border border-stone-200/80 rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 focus:outline-none focus:ring-2 focus:ring-wood-500/20 focus:border-wood-500/30 transition-all placeholder:text-stone-400"
          />
        </div>
        <div>
          <label className="text-[10px] sm:text-xs font-bold text-stone-500 uppercase tracking-widest block mb-2 sm:mb-3">标签颜色</label>
            ...
            {TAG_COLORS.map(color => (
              <button
                ...
                className={cn(
                  ...
                  color,
                  newTagColor === color ? "ring-2 ring-offset-2 ring-wood-500 shadow-sm scale-110" : "hover:shadow-sm"
                )}
              >
        ...
        >
          <Plus size={16} className="mr-2 sm:w-[18px] sm:h-[18px]" /> 创建标签
        </button>
      </form>
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        {tags.length === 0 ? (
          <div className="text-center py-8 sm:py-12 bg-stone-50/50 rounded-2xl border border-stone-200/50 border-dashed">
            <p className="text-xs sm:text-sm text-stone-400 italic">尚未创建标签。</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
            {tags.map(tag => (
                ...
                {editingTagId === tag.id ? (
                  <div className="flex-1 space-y-2 sm:space-y-3 mr-2 sm:mr-3">
                    <input
                      ...
                      className="w-full text-xs sm:text-sm bg-stone-50/50 border border-stone-200/80 rounded-lg px-2.5 py-1.5 sm:px-3 sm:py-2 focus:outline-none focus:ring-2 focus:ring-wood-500/20 transition-all"
                      autoFocus
                    />
                      ...
                      {TAG_COLORS.map(color => (
                        <button
                          ...
                          className={cn(
                            ...
                            color,
                            editTagColor === color ? "ring-2 ring-offset-1 ring-wood-500 scale-110" : ""
                          )}
                        >
              ...
              <div className="flex items-center space-x-0.5 sm:space-x-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                {editingTagId === tag.id ? (
                  <button onClick={saveEdit} className="p-1.5 text-wood-600 hover:bg-wood-50 rounded">

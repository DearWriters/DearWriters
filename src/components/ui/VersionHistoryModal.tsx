  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl animate-in fade-in zoom-in-95 duration-200">
        <h3 className="text-lg font-semibold text-stone-900">版本历史</h3>
        
        {loading || restoring ? (
          <div className="mt-4 text-center text-stone-500">{restoring ? '恢复中...' : '加载版本中...'}</div>
        ) : (
          <div className="mt-4 max-h-96 overflow-y-auto">
            {versions.length === 0 ? (
              <p className="text-sm text-stone-500">未找到历史版本。</p>
            ) : (

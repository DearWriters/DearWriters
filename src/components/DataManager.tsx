...
import { Archive, Download, FileInput, Trash2, X, RotateCcw } from 'lucide-react';
import { cn } from '../lib/utils';
import { toast } from 'sonner';

export function DataManager({ onClose, isTab = false }: { onClose?: () => void; isTab?: boolean }) {
  const { works, deleteWork, mergeData, importData, resetToDefault, state } = useStore(useShallow(state => ({
    works: state.works,
    ...
    reader.onload = (e) => {
      try {
        ...
        if (importedData && typeof importedData === 'object' && 'works' in importedData) {
          ...
          toast.success('完整数据导入成功');
        } else {
          throw new Error('无效的数据格式');
        }
      } catch (err) {
        toast.error('导入完整数据失败');
      } finally {
    ...
    <div className="flex flex-col md:flex-row h-full w-full bg-white overflow-y-auto">
      ...
      <div className="w-full md:w-1/3 md:min-w-[350px] border-b md:border-b-0 md:border-r border-stone-200 flex flex-col bg-stone-50/30">
        <div className="p-4 md:p-6 border-b border-stone-200 bg-white">
          ...
          <p className="text-[10px] md:text-[11px] text-stone-400 uppercase font-bold tracking-widest">管理所有作品</p>
        </div>
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
          <div className="grid grid-cols-1 gap-2 md:gap-3">
            ...
            <label className={cn(
              ...
              {isFullImporting ? '导入中...' : '完整导入'}
              <input type="file" accept=".json" onChange={handleFullImport} className="hidden" disabled={isFullImporting} />
            </label>

            {showResetConfirm ? (
              <div className="mt-4 p-4 bg-red-50 border border-red-100 rounded-xl space-y-3 animate-in fade-in zoom-in-95 duration-200">
                <p className="text-xs text-red-800 font-bold leading-tight">确定要重置所有数据吗？这将清除所有当前作品并恢复到初始演示状态。</p>
              ...
              <button
                ...
                className="flex items-center justify-center px-4 py-2.5 md:py-3 border border-red-200 text-red-600 rounded-xl text-sm font-bold hover:bg-red-50 transition-all shadow-sm active:scale-95 mt-2"
              >
          ...
          <div className="p-3 md:p-4 bg-stone-100/50 rounded-xl border border-stone-200">
            <p className="text-xs text-stone-500 leading-relaxed">
              <strong className="text-stone-700">注意：</strong> 完整导入将覆盖所有当前本地数据。请谨慎使用。导出将创建您所有作品和设置的完整备份。
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-white overflow-y-auto">
        <div className="p-4 md:p-6 border-b border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between bg-white gap-4">
          <div className="space-y-1">
            <h2 className="text-lg md:text-xl font-bold text-stone-900 tracking-tight flex items-center">
              <Archive size={20} className="mr-2 text-wood-600" />
              增量管理
            </h2>
            <p className="text-[10px] md:text-[11px] text-stone-400 uppercase font-bold tracking-widest">管理单个作品</p>
          </div>
          <label className={cn(
          ...
          {workToDelete ? (
            <div className="max-w-md mx-auto mt-8 p-6 bg-red-50 border border-red-100 rounded-2xl text-red-800 shadow-sm">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-lg font-bold">删除作品？</h3>
              </div>
              <p className="mb-6 text-sm leading-relaxed">您确定要归档并删除 <strong className="font-bold">{works.find(w => w.id === workToDelete)?.title}</strong> 吗？这将删除与此作品相关的所有章节、场景和角色。请确保您已先导出它。</p>
    ...
    <div className="w-full max-w-md bg-white rounded-xl shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
      <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-stone-50">
        <h3 className="font-bold text-stone-900 flex items-center uppercase tracking-wider text-xs">
          <Archive size={16} className="mr-2 text-wood-600" />
          Data Management
        </h3>
        {onClose && (
      ...
      <div className="p-6 space-y-8 overflow-y-auto">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {showResetConfirm ? (
              <div className="col-span-2 mt-2 p-4 bg-red-50 border border-red-100 rounded-lg space-y-3 animate-in fade-in zoom-in-95 duration-200">
                <p className="text-xs text-red-800 font-bold leading-tight">Are you sure you want to reset all data? This will clear all current works and restore the initial demo state.</p>
                <div className="flex gap-2">
                  <button
                    onClick={resetToDefault}
                    className="flex-1 py-2 bg-red-600 text-white rounded-md text-xs font-bold hover:bg-red-700 transition-colors"
                  >
                    Confirm Reset
                  </button>
                  <button
                    onClick={() => setShowResetConfirm(false)}
                    className="flex-1 py-2 bg-white border border-red-200 text-red-700 rounded-md text-xs font-bold hover:bg-red-50 transition-colors"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <button
                onClick={() => setShowResetConfirm(true)}
                className="flex items-center justify-center col-span-2 px-4 py-2.5 border border-red-100 text-red-600 rounded-lg text-sm font-medium hover:bg-red-50 transition-colors shadow-sm mt-2"
              >

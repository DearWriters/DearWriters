      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-stone-100 bg-stone-50/50">
          <div className="flex items-center gap-2 text-stone-700 font-medium">
            <Inbox size={20} className="text-wood-600" /> 快速笔记
          </div>
          <button 
            onClick={() => setIsOpen(false)}
        ...
        <div className="p-4">
          <textarea
            ...
            placeholder="你在想什么？(Cmd/Ctrl + Enter 保存)"
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {tagIds.map(tagId => {
              ...
              return tag ? (
                <span key={tag.id} className="px-2 py-0.5 bg-wood-100 text-wood-800 rounded text-xs">{tag.name} <button onClick={() => setTagIds(prev => prev.filter(id => id !== tagId))}><X size={10} /></button></span>
              ) : null;
            })}
        ...
        <div className="p-3 bg-stone-50 border-t border-stone-100 flex justify-between items-center">
          <div className="text-xs text-stone-400">
            按 <kbd className="px-1.5 py-0.5 bg-white border border-stone-200 rounded text-stone-500 font-mono">Cmd/Ctrl + Enter</kbd> 保存
          </div>
          <button
            onClick={handleSave}
            disabled={!content.trim()}
            className="flex items-center gap-1.5 px-4 py-1.5 bg-wood-600 text-white text-sm font-medium rounded-md hover:bg-wood-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Send size={14} />
            保存到笔记
          </button>

...
function LensNoteTextarea({ lens }: { lens: any }) {
  ...
  return (
    <textarea
      ...
      placeholder="在此添加私人笔记、背景设定或想法..."
      className="w-full h-24 p-2 rounded-lg border border-stone-200 bg-stone-50 resize-none outline-none focus:ring-2 focus:ring-wood-500/20 focus:border-wood-500 text-sm text-stone-700"
    />
  );
}
...
export function LensesPanel({ documentId, onClose, onNavigateToBlock }: { documentId: string, onClose: () => void, onNavigateToBlock: (blockId: string) => void }) {
  ...
  const allAvailableTags = Array.from(new Set(allLenses.flatMap(l => l.tags || [])));
  
  const filteredLenses = allLenses.filter(l => {
    const matchesSearch = !searchTerm || (l.notes && l.notes.toLowerCase().includes(searchTerm.toLowerCase())) || 
                          (l.content && l.content.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesTags = selectedFilterTags.length === 0 || (l.tags && selectedFilterTags.every(t => l.tags!.includes(t)));
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-stone-200 bg-white">
        <h4 className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-3">添加透镜</h4>
      ...
      <div className="p-4 border-b border-stone-200 bg-white">
        <div className="relative mb-2">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-stone-50 border border-stone-200 rounded-md text-xs focus:outline-none focus:ring-1 focus:ring-wood-500"
          />
        </div>
        <MultiSelectDropdown
          options={allAvailableTags.map(t => ({ id: t, title: t }))}
          selectedIds={selectedFilterTags}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {filteredLenses.length === 0 ? (
          <div className="text-center text-xs text-stone-500 py-4">未找到透镜。</div>
        ) : (
          filteredLenses.map(lens => (
            <div 
              ...
              className={cn(
                ...
                activeLensId === lens.id && "ring-2 ring-wood-500 ring-offset-1"
              )}
            >
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center ml-2 space-x-1 shrink-0">
                  {lens.linkedLensIds && lens.linkedLensIds.length > 0 && (
                    <div className="flex items-center text-[10px] font-bold bg-black/5 px-1.5 py-0.5 rounded-full text-black/40" title={`${lens.linkedLensIds.length} 个关联透镜`}>
              ...
              {expandedLensId === lens.id && (
                <div className="mt-3 pt-3 border-t border-black/10 space-y-4">
                  ...
                  <div className="mt-4">
                    <label className="block text-[10px] font-bold text-black/40 uppercase tracking-wider mb-2">标签</label>
                    <div className="flex flex-wrap gap-2 mb-2">
                      {(lens.tags || []).map(tag => (
                        <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded flex items-center gap-1 border bg-amber-100 text-amber-800 border-amber-200">
                          {tag}
                          <X size={10} className="cursor-pointer" onClick={() => updateBlock({ id: lens.id, tags: (lens.tags || []).filter(t => t !== tag) })} />
                        </span>
                      ))}
                    </div>
                    <input
                      type="text"
                      placeholder="+ 添加标签 (回车)..."
                      className="w-full text-xs p-1.5 rounded border border-stone-200 bg-white"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          const newTag = e.currentTarget.value.trim();
                          if (newTag && !(lens.tags || []).includes(newTag)) {
                            updateBlock({ id: lens.id, tags: [...(lens.tags || []), newTag] });
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-black/40 uppercase tracking-wider mb-2 flex items-center">
                      <LinkIcon size={10} className="mr-1" /> 关联透镜
                    </label>
                  ...
                  {(() => {
                    ...
                    return (
                      <div>
                        <label className="block text-[10px] font-bold text-black/40 uppercase tracking-wider mb-2">被关联自</label>
                        <div className="flex flex-wrap gap-2">
                          {backlinks.map(backlink => (
                            ...
                            >
                              ...
                              <span className="truncate max-w-[200px]">
                                {backlink.lensColor === 'black' ? '隐藏内容' : (backlink.content || '空透镜')}
                              </span>
                            </button>
                          ))}
                        </div>

...
export function Sidebar({ mobileOpen, setMobileOpen }: { mobileOpen?: boolean, setMobileOpen?: (open: boolean) => void }) {
  ...
  return (
    <>
      ...
      <div className={cn(
        "h-[100dvh] bg-[#2c241b] text-[#d3c5b0] flex flex-col transition-all duration-700 border-r-[4px] border-double border-[#5c4a3d] z-50 shadow-[4px_0_15px_rgba(0,0,0,0.2)]",
        collapsed ? "md:w-20 w-64" : "w-72",
        ...
        <div className="p-4 pt-safe-top flex items-center justify-between border-b-2 border-dashed border-[#5c4a3d] bg-[#231d15]">
          {isExpanded && <span className="font-display text-[#eaddc5] tracking-[0.2em] uppercase text-lg">藏书阁</span>}
          <button 
                  ...
                  <Draggable key={work.id} draggableId={work.id} index={index}>
                    {(provided, snapshot) => (
                      <div
                        ...
                        className={cn(
                          "group relative flex items-center px-4 py-3 text-sm transition-all duration-500 border-b border-[#5c4a3d]/30",
                          activeWorkId === work.id 
                            ? "bg-[#3e3222] text-[#f4ebd8] border-l-[6px] border-l-[#8b0000] shadow-inner" 
                            : "hover:bg-[#3e3222]/50 hover:text-[#eaddc5]",
                          snapshot.isDragging && "bg-[#3e3222] shadow-xl z-50"
                        )}
                      >
                        <div 
                          ...
                          {isExpanded && (
                            editingWorkId === work.id ? (
                              <input
                                ...
                                className="flex-1 bg-stone-700 text-stone-100 px-2 py-0.5 rounded outline-none ring-1 ring-wood-500"
                              />
                            ) : deletingWorkId === work.id ? (
                              <div className="flex items-center space-x-2" onClick={e => e.stopPropagation()}>
                                <span className="text-red-400 font-bold text-xs uppercase">删除?</span>
                                <button 
        ...
        {isExpanded && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-stone-800 text-stone-200 text-sm rounded-md pl-9 pr-3 py-2 outline-none focus:ring-1 focus:ring-wood-500 placeholder:text-stone-500"
            />
          </div>
        )}

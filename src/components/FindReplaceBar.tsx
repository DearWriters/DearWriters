  ...
  return (
    <div className="flex flex-col p-2 bg-stone-100 border-b border-stone-200 animate-in slide-in-from-top-2">
      <div className="flex items-center space-x-2 mb-2">
        <div className="relative flex-1">
            ...
            <input
                ...
                className="w-full pl-8 pr-2 py-1 text-sm border border-stone-300 rounded-md focus:outline-none focus:ring-1 focus:ring-wood-500"
            />
        </div>
        <div className="flex items-center space-x-1">
            <button onClick={handlePrev} className="p-1 hover:bg-stone-200 rounded" title="上一个匹配">
                <ChevronUp size={16} />
            </button>
            <button onClick={handleNext} className="p-1 hover:bg-stone-200 rounded" title="下一个匹配">
                <ChevronDown size={16} />
            </button>
        </div>
      ...
      <div className="flex items-center mt-2 space-x-4 text-xs text-stone-500">
        ...
        <span>
            {matches.length === 0 ? '无匹配项' : `第 ${currentMatchIndex + 1} 项，共 ${matches.length} 项`}
        </span>
      </div>
    </div>
  );

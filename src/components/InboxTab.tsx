      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h1 className="text-3xl font-bold text-stone-800 flex items-center gap-3">
            <Inbox size={32} className="text-wood-600" /> 收件箱
          </h1>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {activeWorkId && (
                <button 
                  ...
                  className={cn(
                    "px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 transition-colors",
                    noteScope === 'work' ? "bg-wood-600 text-white" : "bg-white text-stone-600 border border-stone-200 hover:bg-stone-100"
                  )}
                >
            ...
            <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm mb-8">
              ...
              {noteScope === 'work' && (
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-2 pt-2 border-t border-stone-100">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-stone-500">关联到场景 (可选):</span>
                  </div>
              ...
              <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-stone-100">
                {newTagIds.map(tagId => {
                  ...
                  return tag ? (
                    <span key={tag.id} className="px-2 py-0.5 bg-wood-100 text-wood-800 rounded text-xs flex items-center gap-1">
                      {tag.name} 
                ...
                <div className="relative group">
                  <input 
                    ...
                    className="text-xs bg-stone-100 rounded px-2 py-1 w-full outline-none focus:ring-1 focus:ring-wood-500"
                  />
                  <div className="absolute top-full left-0 bg-white border rounded shadow-lg z-10 w-40 max-h-40 overflow-y-auto hidden group-focus-within:block">
                    <div className="text-[10px] text-stone-500 px-2 py-1">最近使用:</div>
                    ...
                    {tagSearch && (
                      <>
                        <div className="text-[10px] text-stone-500 px-2 py-1 border-t">搜索结果:</div>
                        {filteredTags.length > 0 ? (
                          ...
                          <button 
                            ...
                            className="block w-full text-left px-2 py-1 text-xs text-wood-600 hover:bg-wood-50"
                          >
                            + 添加 "{tagSearch}"
                          </button>
                        )}
              ...
              <div className="flex justify-end mt-2 pt-2 border-t border-stone-100">
                <button onClick={handleAdd} disabled={!newContent.trim()} className="px-4 py-2 bg-wood-600 text-white rounded-lg hover:bg-wood-700 disabled:opacity-50 flex items-center gap-2 font-medium transition-colors">
                  <Plus size={18} /> 保存笔记
                </button>
              </div>
            </div>
                    ...
                    {editingId === item.id ? (
                      <div className="mb-3">
                        <textarea
                          ...
                          className="w-full text-base bg-stone-50 border border-stone-200 rounded p-2 outline-none focus:ring-2 focus:ring-wood-500 min-h-[80px]"
                        />
                        <div className="flex justify-end gap-2 mt-2">
                          <button onClick={() => setEditingId(null)} className="px-3 py-1 text-sm text-stone-500 hover:bg-stone-100 rounded">取消</button>
                          <button onClick={handleSaveEdit} className="px-3 py-1 text-sm bg-wood-600 text-white rounded hover:bg-wood-700">保存</button>
                        </div>
                      </div>
                    ) : (
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex flex-wrap items-center gap-2">
                        {item.sceneId && (
                          <span className="px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded text-xs flex items-center gap-1">
                            <FileText size={12} />
                            {scenes.find(s => s.id === item.sceneId)?.title || '未知场景'}
                          </span>
                        )}
                        ...
                        <div className="relative group/tag">
                          ...
                          <div className="absolute top-full left-0 bg-white border rounded shadow-lg z-10 w-40 max-h-40 overflow-y-auto hidden group-hover/tag:block">
                            ...
                            <div className="text-[10px] text-stone-500 px-2 py-1 border-t">全部:</div>
                            {inboxTags.map(tag => (
                              <button key={tag.id} onClick={() => updateNote({ id: item.id, tagIds: [...(item.tagIds || []), tag.id] })} className="block w-full text-left px-2 py-1 text-xs hover:bg-stone-100">{tag.name}</button>
                            ))}
                          </div>
                      ...
                      <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity self-end sm:self-auto">
                        <div className="relative group/scope">
                          <button className="p-1.5 text-stone-400 hover:text-stone-600 hover:bg-stone-100 rounded" title="更改范围">
                            <Settings size={14} />
                          </button>
                          <div className="absolute bottom-full right-0 mb-1 bg-white border rounded shadow-lg z-10 w-48 hidden group-hover/scope:block p-2">
                            <div className="text-xs font-medium text-stone-700 mb-2">移动笔记至:</div>
                            <button 
                            ...
                            {activeWorkId && (
                              <>
                                ...
                                <div className="mt-2 pt-2 border-t border-stone-100">
                                  <div className="text-[10px] text-stone-500 mb-1 px-2">关联到场景:</div>
                                    ...
                                    {workScenes.map(scene => (
                                      <button 
                                        ...
                                        className={cn(
                                          ...
                                          item.sceneId === scene.id && "bg-wood-50 text-wood-700"
                                        )}
                                      >
                                        {scene.title}
                                      </button>
                        ...
                        <button 
                          ...
                          className="p-1.5 text-stone-400 hover:text-wood-600 hover:bg-wood-50 rounded"
                        >
                          <Edit2 size={14} />
                        </button>

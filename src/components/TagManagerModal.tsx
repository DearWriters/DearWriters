    ...
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-6 border-b border-stone-100 shrink-0">
          <h3 className="text-xl font-bold text-stone-800">管理标签</h3>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form onSubmit={handleAddTag} className="mb-6 space-y-3">
            <div>
              <label className="text-xs font-bold text-stone-400 uppercase tracking-wider block mb-1">新标签名称</label>
              <input
                type="text"
                value={newTagName}
                onChange={(e) => setNewTagName(e.target.value)}
                placeholder="例如：重要，支线A"
                className="w-full text-sm bg-stone-50 border border-stone-200 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-wood-500/20"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-stone-400 uppercase tracking-wider block mb-1">颜色</label>
                ...
                {TAG_COLORS.map(color => (
                  <button
                    ...
                    className={cn(
                      ...
                      color,
                      newTagColor === color ? "ring-2 ring-offset-1 ring-wood-500" : ""
                    )}
                  >
            ...
            >
              <Plus size={16} className="mr-2" /> 添加标签
            </button>
          </form>

          <div className="space-y-2">
            <label className="text-xs font-bold text-stone-400 uppercase tracking-wider block mb-2">现有标签</label>
            {tags.length === 0 ? (
              <p className="text-sm text-stone-500 italic text-center py-4">尚未创建标签。</p>
            ) : (
              tags.map(tag => (
                  ...
                  {editingTagId === tag.id ? (
                    <div className="flex-1 space-y-2 mr-2">
                      <input
                        ...
                        className="w-full text-sm bg-white border border-stone-200 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-wood-500/20"
                        autoFocus
                      />
                        ...
                        {TAG_COLORS.map(color => (
                          <button
                            ...
                            className={cn(
                              ...
                              color,
                              editTagColor === color ? "ring-2 ring-offset-1 ring-wood-500" : ""
                            )}
                          >

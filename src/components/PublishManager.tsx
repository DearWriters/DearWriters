    ...
    <div className={cn(
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="p-4 md:p-6 border-b border-stone-200 bg-white">
          ...
          <p className="text-[10px] md:text-[11px] text-stone-400 uppercase font-bold tracking-widest">管理版本快照</p>
        </div>
      ...
      <div className={cn(
        ...
        <div className="p-4 md:p-6 border-b border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between bg-white gap-4">
          <div className="space-y-1">
            <h2 className="text-lg md:text-xl font-bold text-stone-900 tracking-tight flex items-center">
              <Send size={20} className="mr-2 text-wood-600" />
              发布平台
            </h2>
            <p className="text-[10px] md:text-[11px] text-stone-400 uppercase font-bold tracking-widest">追踪跨平台进度</p>
          </div>
          <button
        ...
        <div className="flex-1 p-4 md:p-8 space-y-6 md:space-y-8 pb-24 md:pb-8">
          {workPlatforms.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-stone-300 space-y-4">
              ...
              <p className="text-sm font-medium">尚未添加平台。</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center space-x-3 md:space-x-4">
                      <div className={cn(
                        ...
                        needsUpdateCount > 0 ? "bg-amber-100 text-amber-600" : "bg-wood-100 text-wood-600"
                      )}>
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-0.5">
                          <span className="text-[10px] font-bold text-wood-600 uppercase tracking-wider bg-wood-50 px-2 py-0.5 rounded-full border border-wood-100 whitespace-nowrap">
                            {Object.values(platform.chapterStatuses).filter(s => s.status === 'published').length} 已发布
                          </span>
                          {needsUpdateCount > 0 && (
                            ...
                            <div key={chapter.id} className={cn(
                              ...
                              <div className="flex-1 min-w-0 mr-4">
                                ...
                                <div className="text-[9px] text-stone-400 mt-0.5 flex flex-wrap items-center gap-2">
                                  {isPublished && <span className="text-wood-600 font-bold whitespace-nowrap">最新: {chapterSnapshots.find(s => s.id === status.lastPublishedSnapshotId)?.versionName}</span>}
                                  {needsUpdate && (
        ...
        <div className="fixed inset-0 bg-stone-900/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] md:h-[80vh] flex flex-col overflow-hidden">
            <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-stone-50">
              <div>
                <h3 className="font-bold text-stone-900">变更详情</h3>
                <p className="text-xs text-stone-500">
                  将当前内容与版本进行比较： <span className="font-bold">{chapterSnapshots.find(s => s.id === diffView.snapshotId)?.versionName}</span>
                </p>
              </div>
                  ...
                  return (
                    <div key={scene.id} className="space-y-4">
                      <div className="flex items-center space-x-2 pb-2 border-b border-stone-100">
                        ...
                        <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded text-[10px] font-bold uppercase">已修改</span>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                        <div className="space-y-2 md:space-y-3">
                          <div className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">快照版本</div>
                        ...
                        <div className="space-y-2 md:space-y-3">
                          <div className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">当前版本</div>
      ...
      {showNewPlatform && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-stone-50">
              <h3 className="font-bold text-stone-900 uppercase tracking-wider text-xs">添加平台追踪</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-bold text-stone-500 uppercase">平台名称</label>
                <input
                  type="text"
                  value={platformName}
                  onChange={(e) => setPlatformName(e.target.value)}
                  placeholder="例如：起点, 知乎, Wattpad"
      ...
      {showNewSnapshot && (
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-stone-50">
              <h3 className="font-bold text-stone-900 uppercase tracking-wider text-xs">创建章节快照</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-bold text-stone-500 uppercase">选择章节</label>
                <select
                  value={selectedChapterId || ''}
                  onChange={(e) => setSelectedChapterId(e.target.value)}
                  className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-stone-900"
                >
                  <option value="">选择一个章节...</option>
                  {workChapters.map(c => (
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-stone-500 uppercase">版本名称</label>
                <input
                  type="text"
                  value={versionName}
                  onChange={(e) => setVersionName(e.target.value)}
                  placeholder="例如：最终稿, v1.2"
                  className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-stone-900"
                />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-stone-500 uppercase">备注（可选）</label>
                <textarea
                  value={snapshotNote}

    ...
    <div ref={containerRef} className="flex flex-col md:flex-row h-full overflow-hidden">
      ...
      {!isMobile && (
        <div 
          ...
          className="w-1.5 shrink-0 cursor-col-resize bg-stone-200 hover:bg-wood-400 active:bg-wood-500 transition-colors z-10"
        />
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 bg-stone-50/30 custom-scrollbar">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 bg-white/50 backdrop-blur-sm p-3 sm:p-4 rounded-xl border border-stone-200/60 shadow-sm sticky top-0 z-20 gap-3 sm:gap-4">
          <h3 className="text-sm font-bold text-stone-800 uppercase tracking-wider flex items-center gap-2 shrink-0">
            <LinkIcon size={16} className="text-wood-600" />
            <span>叙事流</span>
          </h3>
          
          <div className="flex-1 w-full sm:max-w-md mx-auto flex items-center gap-2">
            <span className="text-[10px] font-bold text-stone-400 uppercase tracking-wider shrink-0 hidden sm:inline">跳转至:</span>
            <select 
              className="w-full text-xs bg-white border border-stone-200 rounded-lg px-3 py-1.5 text-stone-600 focus:outline-none focus:ring-2 focus:ring-wood-500/20 focus:border-wood-500 shadow-sm cursor-pointer"
              onChange={(e) => {
                if (e.target.value) {
          ...
          <label className="flex items-center justify-center gap-2 text-xs font-medium text-stone-500 cursor-pointer hover:text-stone-800 transition-colors bg-white px-2.5 py-1.5 rounded-lg border border-stone-200 shadow-sm shrink-0 w-full sm:w-auto" title="显示已归档">
            <input 
              type="checkbox" 
              checked={showArchived} 
              onChange={(e) => setShowArchived(e.target.checked)}
              className="rounded border-stone-300 text-wood-600 focus:ring-wood-500 transition-colors"
            />
            <Archive size={14} className={showArchived ? "text-wood-600" : "text-stone-400"} />
            <span className="hidden sm:inline">显示已归档</span>
            <span className="sm:hidden">已归档</span>
          </label>
        </div>
          ...
          {chapters.map(chapter => {
            ...
            return (
              <div key={chapter.id} id={`chapter-${chapter.id}`} className={cn("space-y-4 sm:space-y-5 scroll-mt-24", chapter.archived && "opacity-60 grayscale-[0.5]")}>
                <h4 
                  className="font-bold text-stone-800 text-base sm:text-lg border-b-2 border-stone-200/60 pb-2 sm:pb-3 flex items-center gap-2 sm:gap-3 cursor-pointer hover:text-wood-600 hover:border-wood-200 transition-colors group"
                  onClick={() => setActiveDocument(chapter.id)}
                >
                  <span className="bg-stone-100 text-stone-500 group-hover:bg-wood-50 group-hover:text-wood-600 px-2 py-1 sm:px-2.5 sm:py-1 rounded-md text-[10px] sm:text-xs tracking-widest uppercase transition-colors">章节</span>
                  {chapter.title}
                  {chapter.archived && <span className="text-[10px] uppercase bg-stone-200 text-stone-600 px-2 py-0.5 rounded-md font-bold ml-auto">已归档</span>}
                </h4>
                  ...
                  {chapterScenes.map(scene => (
                    <div 
                      ...
                      className={cn(
                        ...
                        activeDocumentId === scene.id ? "border-wood-400 ring-2 ring-wood-400/20 shadow-md z-30" : "border-stone-200/80 hover:border-stone-300 hover:shadow-md z-10"
                      )}
                    ...
                    >
                      <div className="flex items-center justify-between mb-4 sm:mb-6">
                        <h5 className="font-bold text-stone-800 flex items-center gap-2 text-sm sm:text-base">
                          <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-stone-300 group-hover/scene:bg-wood-400 transition-colors" />
                          {scene.title}
                        </h5>
                        <span className="text-[9px] sm:text-[10px] font-bold text-stone-400 uppercase tracking-widest bg-stone-50 px-2 py-1 rounded-md border border-stone-100">场景</span>
                      </div>

                      {/* Event Selection */}
                      <div className="mb-4 sm:mb-6 relative z-20">
                        <label className="text-[9px] sm:text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-2 block">关联事件</label>
                        <MultiSelectDropdown
                      ...
                      {(scene.linkedEventIds || []).length > 0 && (
                        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-stone-100 relative z-10">
                          <div className="flex items-center gap-2 mb-3 sm:mb-4">
                            <Users size={14} className="text-wood-600" />
                            <label className="text-[9px] sm:text-[10px] font-bold text-stone-400 uppercase tracking-wider block">角色行动矩阵</label>
                          </div>
                          ...
                          <div className="overflow-x-auto custom-scrollbar -mx-2 px-2">
                            <div className="min-w-full inline-block align-middle">
                              <table className="min-w-full border-collapse">
                                <thead>
                                  <tr>
                                    <th className="sticky left-0 z-10 bg-white border-b border-stone-200 p-2 text-left text-[9px] sm:text-[10px] font-bold text-stone-400 uppercase tracking-wider min-w-[100px] sm:min-w-[120px] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">角色</th>
                                    {(() => {
                                          ...
                                          {linkedEvents.map(event => (
                                            <td key={event.id} className="border-b border-stone-100 p-2 align-top">
                                              <AutoResizeTextarea
                                                ...
                                                className="w-full bg-stone-50/50 border border-transparent hover:border-stone-200 focus:bg-white focus:border-wood-500 rounded p-2 text-stone-600 focus:ring-0 resize-none overflow-hidden min-h-[3rem] placeholder:text-stone-300 text-[10px] sm:text-xs transition-all"
                                              />
                                            </td>
                                          ))}
                                        </tr>

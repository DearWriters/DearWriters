      ...
      <div className="p-4 border-b border-stone-200 bg-white">
        ...
        <div className="space-y-2">
          <textarea
            ...
            placeholder={viewMode === 'document' && sceneId ? "为此场景添加笔记..." : "添加作品级笔记..."}
            className="w-full p-2 text-sm border border-stone-200 rounded-md focus:ring-2 focus:ring-wood-500 focus:border-transparent outline-none resize-none h-20"
          />
          <div className="flex items-center justify-between">
            <div className="flex flex-wrap gap-1">
              {selectedTagIds.map(tagId => {
                ...
                return tag ? (
                  <span key={tag.id} className="px-1.5 py-0.5 bg-wood-100 text-wood-800 rounded text-[10px] flex items-center gap-1">
                    {tag.name}
            ...
            <button
              ...
              className="px-3 py-1 bg-wood-600 text-white text-xs font-medium rounded hover:bg-wood-700 disabled:opacity-50 transition-colors flex items-center gap-1"
            >
              <Plus size={14} /> 添加
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {filteredNotes.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-stone-400">未找到笔记。</p>
          </div>
        ) : (
              ...
              {editingId === note.id ? (
                <div className="space-y-2">
                  <textarea
                    ...
                    className="w-full p-2 text-sm border border-stone-200 rounded outline-none focus:ring-1 focus:ring-wood-500"
                    autoFocus
                  />
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={handleSaveEdit} className="p-1 text-wood-600 hover:text-wood-700">
                      <Check size={16} />
                    </button>
                  </div>
                </div>

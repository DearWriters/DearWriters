    ...
    if (activeTab === 'characters' && initialCharacterId) {
      const timer = setTimeout(() => {
        ...
        if (element) {
          ...
          element.classList.add('ring-2', 'ring-wood-500', 'ring-offset-2');
          setTimeout(() => {
            element.classList.remove('ring-2', 'ring-wood-500', 'ring-offset-2');
          }, 2000);
        }
      }, 100);
      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[600px] lg:w-[800px] bg-white shadow-2xl z-[70] flex flex-col transform transition-transform duration-300 ease-in-out translate-x-0 border-l border-stone-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 pb-0 border-b border-stone-100 relative bg-white z-20 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            {/* Timestamp Badge */}
            <div className="flex items-center gap-1.5 bg-stone-100/80 hover:bg-stone-100 px-3 py-1.5 rounded-lg border border-stone-200/50 transition-colors focus-within:ring-2 focus-within:ring-wood-500/20">
          ...
          <div className="flex gap-6 border-b border-stone-200">
            <button 
              className={cn("pb-3 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors", activeTab === 'general' ? "border-wood-500 text-wood-700" : "border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300")}
              onClick={() => setActiveTab('general')}
            >
              <FileText size={16} /> 详情
            </button>
            <button 
              className={cn("pb-3 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors", activeTab === 'characters' ? "border-wood-500 text-wood-700" : "border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300")}
              onClick={() => setActiveTab('characters')}
            >
              <Users size={16} /> 角色
              {characterCount > 0 && (
                <span className={cn("py-0.5 px-2 rounded-full text-[10px]", activeTab === 'characters' ? "bg-wood-100 text-wood-700" : "bg-stone-100 text-stone-600")}>
                  {characterCount}
                </span>
              )}
            </button>
            <button 
              className={cn("pb-3 text-sm font-medium flex items-center gap-2 border-b-2 transition-colors", activeTab === 'relations' ? "border-wood-500 text-wood-700" : "border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300")}
              onClick={() => setActiveTab('relations')}
            >
              <Network size={16} /> 关系
              {relationCount > 0 && (
                <span className={cn("py-0.5 px-2 rounded-full text-[10px]", activeTab === 'relations' ? "bg-wood-100 text-wood-700" : "bg-stone-100 text-stone-600")}>
                  {relationCount}
                </span>
              )}
            </button>
            ...
            <div className="h-full flex flex-col gap-6">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-widest block mb-2">开始时间</label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-stone-50 border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-wood-500/20 transition-all"
                    placeholder="池"
                  />
                </div>
                <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-widest block mb-2">持续时间</label>
                  <input
                    type="number"
                    value={localDuration}
                ...
                <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-widest block mb-2">重要性 (1-5)</label>
                  <div className="flex items-center gap-2">
                    {[1, 2, 3, 4, 5].map((val) => (
                      <button
                        key={val}
                        ...
                        className={cn(
                          ...
                          localImportance === val 
                            ? "bg-wood-500 text-white shadow-md scale-110" 
          ...
          {activeTab === 'characters' && (
            <div className="space-y-6">
              <div className="bg-white p-5 rounded-xl border border-stone-200 shadow-sm">
                <label className="text-xs font-bold text-stone-400 uppercase tracking-wider block mb-3">涉及角色</label>
                <MultiSelectDropdown
              ...
              <div className="space-y-4">
                {characterCount === 0 ? (
                  <div className="text-center py-12 px-4 border-2 border-dashed border-stone-200 rounded-xl bg-stone-50/50">
                    ...
                    <h4 className="text-sm font-medium text-stone-600 mb-1">没有涉及的角色</h4>
                    <p className="text-xs text-stone-400">在上方选择角色以描述他们在此事件中的行动。</p>
                  </div>
                ) : (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-bold text-stone-800">{character?.name || '未知'}</span>
                            <button 
                          ...
                          <textarea
                            ...
                            className="text-sm bg-stone-50/50 hover:bg-stone-50 focus:bg-white border border-transparent hover:border-stone-200 focus:border-stone-300 rounded-lg px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-wood-500/20 resize-none transition-all"
                            placeholder={`${character?.name || '这个角色'} 在做什么？`}
                            rows={2}
                          />
                        </div>
                      </div>
            ...
            <div className="space-y-8">
              ...
              <div>
                <h4 className="text-sm font-bold text-stone-800 mb-4 flex items-center gap-2">
                  <LinkIcon size={16} className="text-wood-600" /> 关联事件
                </h4>
                <div className="bg-white p-5 rounded-xl border border-stone-200 shadow-sm">
                  <p className="text-xs text-stone-500 mb-4">将此事件与其他相关事件关联（例如，因果关系，主题联系）。</p>
                  <MultiSelectDropdown
              ...
              <div>
                <h4 className="text-sm font-bold text-stone-800 mb-4 flex items-center gap-2">
                  <Network size={16} className="text-wood-600" /> 水平（固定间隔）
                </h4>
                <div className="bg-white p-5 rounded-xl border border-stone-200 shadow-sm">
                  <p className="text-xs text-stone-500 mb-4">水平固定事件以保持固定间隔。</p>
                  <MultiSelectDropdown
                    options={allEvents.filter(e => e.id !== event.id).map(e => ({ id: e.id, title: e.title }))}
              ...
              <div>
                <h4 className="text-sm font-bold text-stone-800 mb-4 flex items-center gap-2">
                  <Network size={16} className="text-wood-600" /> 垂直（同步/堆叠）
                </h4>
                <div className="bg-white p-5 rounded-xl border border-stone-200 shadow-sm">
                  <p className="text-xs text-stone-500 mb-4">垂直同步事件以堆叠它们。</p>
                  <MultiSelectDropdown
                    options={allEvents.filter(e => e.id !== event.id).map(e => ({ id: e.id, title: e.title }))}

...
const DEFAULT_COLUMNS: ColumnConfig[] = [
  { id: 'timestamp', label: '时间戳', visible: true, width: 'w-32' },
  { id: 'event', label: '事件', visible: true, width: 'w-64' },
  { id: 'characters', label: '角色', visible: true, width: 'w-48' },
  { id: 'tags', label: '标签', visible: true, width: 'w-48' },
  { id: 'startTime', label: '开始时间', visible: true, width: 'w-24' },
  { id: 'duration', label: '持续时间', visible: true, width: 'w-24' },
];
    ...
    >
      ...
      <button 
        ...
        className={cn(
          ...
          column.visible ? "text-wood-600 hover:bg-wood-50" : "text-stone-400 hover:bg-stone-100"
        )}
      >
    ...
    if (event.horizontalIds) {
      for (const relId of event.horizontalIds) {
        ...
        if (startTime < relStartTime + relDuration && relStartTime < startTime + duration) {
          return { 
            isValid: false, 
            error: `此更改与相关事件重叠: "${relEvent.title || '未命名事件'}"` 
          };
        }
      }
    }
            ...
            <div className="flex items-center gap-2">
              ...
              {hasRelations && (
                <button 
                  ...
                  className={cn(
                    "p-1 rounded transition-colors",
                    isHighlighted ? "text-wood-600 bg-wood-100" : "text-stone-400 hover:text-stone-600 hover:bg-stone-100"
                  )}
                  title="高亮相关事件"
                >
                  <Network size={14} />
    ...
    <div className="flex-1 overflow-auto p-2 sm:p-4 md:p-6 pt-4 md:pt-6 pb-[50vh] flex flex-col gap-4 custom-scrollbar">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-2 border-b border-stone-100">
          <div className="relative flex-1">
            ...
            <input 
              type="text" 
              placeholder="搜索事件..." 
              className="w-full pl-10 pr-4 py-2 bg-stone-50/50 rounded-lg border border-stone-200 text-sm focus:ring-2 focus:ring-wood-500 outline-none transition-all"
              value={filters.text}
          ...
          <button 
            ...
            className={cn(
              ...
              isFilterExpanded ? "bg-wood-50 border-wood-200 text-wood-700" : "bg-white border-stone-200 text-stone-600 hover:bg-stone-50"
            )}
          >
            <Filter size={16} />
            筛选
          ...
          <div className="p-4 bg-stone-50/30 border-b border-stone-100 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">角色</label>
                  <MultiSelectDropdown
                    options={characters.filter(c => c.workId === activeWorkId).map(c => ({ id: c.id, title: c.name }))}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">地点</label>
                  <MultiSelectDropdown
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">标签</label>
                  <MultiSelectDropdown
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">事件类型</label>
                  <div className="flex flex-wrap gap-1 p-1 bg-stone-100 rounded-lg">
                    {(['all', 'timeline', 'pool'] as const).map((status) => (
                      ...
                      >
                        {status === 'all' ? '全部' : status === 'timeline' ? '时间线' : '事件池'}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">开始时间范围</label>
                  <div className="flex gap-2">
                    <input type="number" placeholder="最小" className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm bg-white min-w-0" value={filters.startTimeMin} onChange={(e) => setFilters(prev => ({ ...prev, startTimeMin: e.target.value }))} />
                    <input type="number" placeholder="最大" className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm bg-white min-w-0" value={filters.startTimeMax} onChange={(e) => setFilters(prev => ({ ...prev, startTimeMax: e.target.value }))} />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">持续时间范围</label>
                  <div className="flex gap-2">
                    <input type="number" placeholder="最小" className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm bg-white min-w-0" value={filters.durationMin} onChange={(e) => setFilters(prev => ({ ...prev, durationMin: e.target.value }))} />
                    <input type="number" placeholder="最大" className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm bg-white min-w-0" value={filters.durationMax} onChange={(e) => setFilters(prev => ({ ...prev, durationMax: e.target.value }))} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-end gap-3 mt-6 pt-4 border-t border-stone-200/60">
              ...
              <button 
                ...
                className="w-full sm:w-auto px-6 py-2 bg-wood-600 text-white rounded-lg text-sm font-medium hover:bg-wood-700 transition-all shadow-sm active:scale-95"
              >
                应用筛选
              </button>
            </div>
      ...
      <div className="flex justify-end gap-2 relative w-full max-w-7xl mx-auto shrink-0">
        <button 
          ...
          className={cn(
            ...
            showSettings ? "bg-wood-50 border-wood-200 text-wood-700" : "bg-white border-stone-200 text-stone-600 hover:bg-stone-50 hover:border-stone-300"
          )}
        >
          <Settings2 size={14} />
        ...
        {showSettings && (
          <div className="absolute top-full right-0 mt-2 w-56 bg-white border border-stone-200/80 rounded-xl shadow-xl z-[60] p-3 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-3 px-1">
              <span className="text-xs font-bold text-stone-800 uppercase tracking-wider">管理列</span>
              ...
              <tr className="bg-stone-50/80 border-b border-stone-200/80 text-[11px] font-bold text-stone-500 uppercase tracking-wider">
                <th className="px-4 sm:px-5 py-3 sm:py-3.5 w-10 sticky left-0 bg-stone-50/80 z-10">
                  <input
                    ...
                    className="rounded border-stone-300 text-wood-600 focus:ring-wood-500"
                  />
                </th>
                ...
                <th className="px-4 sm:px-5 py-3 sm:py-3.5 w-16 text-right sticky right-0 bg-stone-50/80 backdrop-blur-sm shadow-[-4px_0_12px_rgba(0,0,0,0.02)]">操作</th>
              </tr>
            </thead>
                ...
                sortedEvents.map((event, idx) => (
                      <tr 
                        ...
                        className={cn(
                          ...
                          "bg-white",
                          (highlightedEventId === event.id || highlightedIds.has(event.id)) ? "bg-wood-50/80" : "hover:bg-stone-50/80",
                          event.startTime === undefined && "opacity-60 grayscale-[0.5]",
                          selectedEventIds.has(event.id) && "bg-wood-50"
                        )}
                      >
                        ...
                        <td className="px-4 sm:px-5 py-3 sm:py-4 align-top text-right sticky right-0 bg-white group-hover:bg-stone-50/80 transition-colors duration-200 shadow-[-4px_0_12px_rgba(0,0,0,0.02)]">
                          <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                            <button onClick={() => setSelectedEventId(event.id)} className="p-1.5 text-stone-400 hover:text-stone-700 hover:bg-stone-100 rounded-md transition-colors" title="查看详情"><Maximize2 size={14} /></button>
      ...
      {selectedEventIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-stone-900 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-4 z-[100] animate-in slide-in-from-bottom-10">
          <span className="text-sm font-medium">已选择 {selectedEventIds.size} 项</span>
          <div className="w-px h-4 bg-stone-700" />
          <button onClick={batchDelete} className="text-sm text-red-400 hover:text-red-300 font-medium">删除</button>
          <div className="w-px h-4 bg-stone-700" />
          <button onClick={() => setIsBatchModalOpen(true)} className="text-sm text-wood-400 hover:text-wood-300 font-medium">分配角色和标签</button>
        </div>
      )}
      {isBatchModalOpen && (
        <BatchActionModal

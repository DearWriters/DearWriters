        ...
        >
          ...
          <div className="max-h-40 overflow-y-auto py-1">
            {filteredOptions.length === 0 && !onCreateOption ? (
              <div className="px-3 py-2 text-xs text-stone-500 italic">无结果</div>
            ) : (
              <>
                {filteredOptions.map(opt => (
                  ...
                  >
                    ...
                    {selectedIds.includes(opt.id) && <Check size={12} className="text-wood-600 shrink-0" />}
                  </div>
                ))}
                {onCreateOption && search.trim() !== '' && !options.some(opt => opt.title.toLowerCase() === search.trim().toLowerCase()) && (
                  <div 
                    className="px-3 py-1.5 text-xs hover:bg-stone-50 cursor-pointer flex items-center text-wood-600"
                    onMouseDown={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      onCreateOption(search.trim());

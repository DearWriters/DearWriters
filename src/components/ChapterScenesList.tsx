  ...
  return (
    <div className="mb-12 space-y-4">
      <div className="flex items-center justify-between border-b border-stone-100 pb-2">
        <h3 className="text-sm font-bold text-stone-400 uppercase tracking-wider">本章场景</h3>
        <button
          onClick={() => addScene({ chapterId: activeDocId })}
          className="text-xs flex items-center gap-1 text-wood-600 hover:text-wood-700 hover:bg-wood-50 px-2 py-1 rounded transition-colors"
        >
          <Plus size={14} />
          添加场景
        </button>
              ...
              >
                ...
                >
                  <div className="flex items-center justify-end mb-2">
                    <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">场景 {scene.order + 1}</span>
                  </div>
                  ...
                  <div className="text-[10px] text-stone-500 flex items-center opacity-60">
                    ...
                    {allBlocks.filter(b => b.documentId === scene.id && !b.isLens).reduce((acc, b) => acc + b.content.length, 0)} 字符
                  </div>
                </button>

...
import { Layers, MapPin, Edit2, Link as LinkIcon, X, Plus, Lock, Filter, ExternalLink, Search, Pin, LayoutGrid, Network } from 'lucide-react';
...
export function LensesTab({ isSubTab }: { isSubTab?: boolean }) {
  ...
  if (!activeWorkId) return <div className="flex-1 flex items-center justify-center text-stone-400">选择一个作品</div>;
  ...
  const getLensLocation = (docId: string) => {
    ...
    if (scene) {
      ...
      return `${chapter?.title || '未知章节'} > ${scene.title}`;
    }
    const chapter = chapters.find(c => c.id === docId);
    return chapter?.title || '未知位置';
  };
  ...
  const scrollToLens = (lensId: string) => {
    ...
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'center' });
      el.classList.add('ring-4', 'ring-wood-500', 'ring-offset-2');
      setTimeout(() => el.classList.remove('ring-4', 'ring-wood-500', 'ring-offset-2'), 2000);
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col mb-8 space-y-6">
            <div className="max-w-2xl flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-serif font-semibold text-stone-900">色彩透镜</h2>
                <textarea
                  ref={descriptionRef}
                  value={activeWork?.lensesDescription ?? "所有高亮信息的全局摘要。"}
                  onChange={(e) => {
                  ...
                  className="text-base md:text-sm text-stone-500 mt-1 bg-transparent border-b border-transparent hover:border-stone-300 focus:border-wood-500 outline-none w-full transition-colors resize-none overflow-hidden block"
                  placeholder="输入透镜的描述..."
                  rows={1}
              ...
              <div className="flex bg-stone-200/50 p-1 rounded-lg ml-4">
                <button
                  ...
                  className={cn(
                    ...
                    viewMode === 'grid' ? "bg-white text-wood-700 shadow-sm" : "text-stone-500 hover:text-stone-700"
                  )}
                  title="网格视图"
                >
                  <LayoutGrid size={18} />
                ...
                <button
                  ...
                  className={cn(
                    "p-1.5 rounded-md transition-all",
                    viewMode === 'graph' ? "bg-white text-wood-700 shadow-sm" : "text-stone-500 hover:text-stone-700"
                  )}
                  title="关系图视图"
                >
                  <Network size={18} />
            ...
            <div className="space-y-4">
              {/* Search Line */}
              <div className="flex items-center space-x-2 bg-white border border-stone-200 rounded-lg px-4 py-2.5 shadow-sm w-full focus-within:border-wood-500 focus-within:ring-1 focus-within:ring-wood-500 transition-all">
              ...
              <div className="flex flex-col space-y-4 bg-stone-100/50 p-4 rounded-xl border border-stone-200/60">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mr-2 flex items-center">
                    <Filter size={10} className="mr-1" /> 章节
                  </span>
                  <button 
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "px-3 py-1 text-xs rounded-full border transition-all",
                          isSelected 
                            ? "bg-wood-100 border-wood-500 text-wood-700 font-bold shadow-sm" 
                            : "bg-white border-stone-200 text-stone-600 hover:border-stone-300"
                        )}
                      >
                        {chap.title}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mr-2 flex items-center">
                    <Layers size={10} className="mr-1" /> 颜色
                  </span>
                  <button 
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected 
                            ? "bg-white border-wood-500 text-wood-700 font-bold shadow-sm ring-1 ring-wood-500/20" 
                            : "bg-white border-stone-200 text-stone-600 hover:border-stone-300"
                        )}
                      >
                        <div 
          ...
          <div className="space-y-10">
            {viewMode === 'graph' ? (
              <div className="h-[600px] w-full">
                <GraphTab showTitle={false} />
              </div>
            ) : (
              <>
                {pinnedLenses.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center space-x-2 text-wood-700">
                  <Pin size={18} />
                  <h3 className="text-lg font-serif font-semibold">已固定的透镜</h3>
                </div>
                  ...
                  {pinnedLenses.map(lens => (
                    <div 
                      ...
                      className={cn(
                        ...
                        selectedLensId === lens.id && "ring-2 ring-wood-500 ring-offset-2 shadow-md"
                      )}
                    ...
                    >
                      <div className="flex justify-between items-start mb-3 pb-2 border-b border-black/10">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className={cn("p-1 rounded transition-colors", lens.pinned ? "text-wood-600 bg-wood-100" : "text-stone-400 hover:text-wood-600 hover:bg-black/5")}
                            title={lens.pinned ? "取消固定透镜" : "固定透镜"}
                          >
                            <Pin size={14} />
                          </button>
                          <button
                            ...
                            className="text-stone-500 hover:text-wood-700 p-1 hover:bg-black/5 rounded transition-colors"
                            title="跳转到文本中的位置"
                          >
                      ...
                      <div className="text-sm leading-relaxed font-medium line-clamp-6 mb-4">
                        {lens.lensColor === 'black' ? (
                          <span className="text-stone-500 italic flex items-center"><Lock size={14} className="mr-1"/> 隐藏内容</span>
                        ) : (
                          lens.content || <span className="italic opacity-50">空透镜...</span>
                        )}
                      </div>

                      {lens.notes && (
                        <div className="mb-4 p-3 bg-white/50 rounded-lg text-xs text-stone-700 whitespace-pre-wrap border border-black/5">
                          <span className="font-bold block mb-1 opacity-70">私人笔记:</span>
                          {lens.notes}
                        </div>
                      )}
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="truncate max-w-[150px]">
                                  {linkedLens.lensColor === 'black' ? '隐藏内容' : (linkedLens.content || '空透镜')}
                                </span>
                              </button>
                            );
                          })}
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-black/10">
                        <div className="flex items-center space-x-3 text-xs font-semibold opacity-70">
                          {lens.notes && (
                            <span className="flex items-center"><Edit2 size={12} className="mr-1" /> 笔记</span>
                          )}
                          {lens.linkedLensIds && lens.linkedLensIds.length > 0 && (
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-serif font-semibold text-stone-900">所有透镜</h3>
              <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
            ...
            return (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8 pb-24 md:pb-6">
                  {/* Color Selection */}
                  <div>
                    <label className="block text-xs font-bold text-stone-400 uppercase tracking-wider mb-2">颜色</label>
                  ...
                  {/* Content Edit */}
                  <div>
                    <label className="block text-xs font-bold text-stone-400 uppercase tracking-wider mb-2">内容</label>
                    <textarea
                      value={lens.content || ''}
                      onChange={(e) => handleUpdateLens(lens.id, { content: e.target.value })}
                      placeholder={lens.lensColor === 'black' ? "隐藏内容..." : "输入透镜内容..."}
                      className={cn(
                        ...
                        LENS_COLORS[lens.lensColor as keyof typeof LENS_COLORS] || LENS_COLORS.red,
                        lens.lensColor === 'black' ? "text-transparent focus:text-stone-100 placeholder:text-stone-700 focus:placeholder:text-stone-500 selection:bg-stone-700 selection:text-stone-100" : "focus:ring-2 focus:ring-wood-500/20"
                      )}
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-stone-400 uppercase tracking-wider mb-2 flex items-center justify-between">
                      <span>私人笔记</span>
                      <span className="text-[10px] bg-stone-100 text-stone-500 px-1.5 py-0.5 rounded">不在编辑器中显示</span>
                    </label>
                    <textarea
                      value={lens.notes || ''}
                      onChange={(e) => handleUpdateLens(lens.id, { notes: e.target.value })}
                      placeholder="在此处添加私人笔记、设定或想法..."
                      className="w-full h-32 p-3 rounded-lg border border-stone-200 bg-stone-50 resize-none outline-none focus:ring-2 focus:ring-wood-500/20 focus:border-wood-500 text-base md:text-sm text-stone-700"
                    />
                  </div>

                  {/* Linking */}
                  <div>
                    <label className="block text-xs font-bold text-stone-400 uppercase tracking-wider mb-2">关联透镜</label>
                    <MultiSelectDropdown

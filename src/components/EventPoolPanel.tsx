  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
                        ...
                        {linkedEvents.map((event) => (
                          <div key={event.id} className="flex flex-col gap-1 group/action">
                            ...
                            <AutoResizeTextarea
                              ...
                              className="w-full bg-white border border-stone-200 rounded p-1.5 text-stone-600 focus:ring-1 focus:ring-wood-500 resize-none overflow-hidden min-h-[2rem] placeholder:text-stone-300 text-xs"
                            />
                          </div>
                        ))}
                      </div>
        ...
        {/* Linked Events */}
        <div>
          <h4 className="text-xs font-bold text-wood-600 uppercase tracking-wider mb-3 flex items-center">
            <LinkIcon size={12} className="mr-1.5" />
            编辑关联事件 ({linkedEvents.length})
          </h4>
          <div className="space-y-2">
            {linkedEvents.map(event => {
              return (
                <div key={event.id} className="bg-white border border-wood-200 rounded-lg p-3 shadow-sm relative group">
                  ...
                  <div className="absolute top-2 right-2 flex gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1.5 text-stone-400 hover:text-wood-600 hover:bg-wood-50 rounded-md transition-colors"
                      title="编辑事件"
                    >
                      <Pencil size={14} />
                    </button>
                    <button
                      ...
                      className="p-1.5 text-wood-600 hover:bg-wood-50 rounded-md transition-colors"
                      title="取消关联"
                    >
                      <Unlink size={14} />
                    </button>
            ...
            {linkedEvents.length === 0 && (
              <div className="text-xs text-stone-400 italic">暂无关联事件。</div>
            )}
          </div>
        </div>
      </div>
